...
def main():
    ...
    if len(envs.observation_space.shape) == 3:
        actor_critic = CNNPolicy(obs_shape[0], envs.action_space, args.recurrent_policy, args.autoencoder_uncertainty)
    else:
        ...
        if args.algo in ['a2c', 'acktr']:
            ...
            if args.autoencoder_uncertainty :
                values, action_log_probs, dist_entropy, states, u_loss = actor_critic.evaluate_actions(
                    ...
                    next_state_target= Variable(rollouts.observations[1:].view(-1, *obs_shape)), action_space = envs.action_space.n)
            else :
            ...
            optimizer.zero_grad()
            (u_loss[0] + value_loss * args.value_loss_coef + action_loss - dist_entropy * args.entropy_coef).backward()
